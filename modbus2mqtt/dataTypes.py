...
# Added new reference types uint8LSB and uint8MSB and related handlers. They work in rw and r mode only.
...
# is stored for both to be able to calclate the whole 16bit to be written to modbus from value received through MQTT (parse-function) which 
# cover 8 bit out of 16 only. For this purpose the additional attribute rawValue gets initialized and is used for uint8LSB and uint8MSB only.
...
class DataTypes:
    ...
    def parseDataType(refobj,conf):
        if conf is None or conf == "uint16" or conf == "":
            ...
            refobj.regAmount=1
            refobj.parse=DataTypes.parseuint8LSB
            refobj.combine=DataTypes.combineuint8LSB
            refobj.rawValue=None
        elif conf == "uint8MSB":
            refobj.regAmount=1
            refobj.parse=DataTypes.parseuint8MSB
            refobj.combine=DataTypes.combineuint8MSB
            refobj.rawValue=None
        elif conf == "int16":
            refobj.regAmount=1
